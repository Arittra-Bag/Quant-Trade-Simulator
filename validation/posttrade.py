...
Usage:
    python -m validation.posttrade validation/data/okx.jsonl [--notional 250000] [--horizon 60] \\
...
def format_report(summary, source, notional, horizon, slices):
    t, p = summary["twap"], summary["passive"]
    head = "| Measure | n | Median | p10 to p90 | Mean |\n| --- | ---: | ---: | ---: | ---: |"
    lines = [
        "# Post-trade scoring",
        "",
        f"Source: `{source}`. {summary['windows']} non-overlapping windows of {horizon:.0f} s, both sides, "
        f"${notional:,.0f} per order. Generated by `python -m validation.posttrade`; the method, and what "
        "it cannot say, is in that module's docstring.",
        "",
        ...
        "",
        "The tape's fill is a floor: trades between polls can be missed, and the queue ahead is assumed "
        "never to cancel.",
        "",
...
def main(argv=None):
    ...
    p.add_argument("path")
    p.add_argument("--notional", type=float, default=250_000.0)
    ...
    summary = summarise(score(books, trades, a.notional, a.horizon, a.slices))
    report = format_report(summary, a.path, a.notional, a.horizon, a.slices)
    if a.report:
        with open(a.report, "w") as fh:
            fh.write(report)
    print(report)
    return 0 if summary["windows"] else 1
