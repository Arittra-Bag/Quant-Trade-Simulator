...
def plans_to_price(advice):
    ...
    if strategy == "iceberg":
        advised = ("twap", advised[1])  # priced as the equal slices it shows
...
def price_plan(order, book, plan):
    ...
    if plan["strategy"] == "twap":
        quote = tools.compare_schedule(side, notional, plan["slices"])
        cost, complete, label = quote["sliced_net_bps"], quote["one_shot_complete"], f"TWAP x{plan['slices']}"
    else:
